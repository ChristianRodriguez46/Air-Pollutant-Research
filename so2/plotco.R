...
# ───────────────────────────────────────────────────────────────
ann_pct_so2 <- so2wt_daily_complete %>%
...
# ───────────────────────────────────────────────────────────────
so2wt_daily_complete <- so2wt_daily_complete %>%
  mutate(date = as.Date(date))
...
# ───────────────────────────────────────────────────────────────
# 5.  Plot base with shading and SO2 series
# ───────────────────────────────────────────────────────────────
p_so2 <- ggplot() +
  geom_rect(data = shade_so2,
            aes(xmin = start, xmax = end,
                ymin = -Inf,  ymax =  Inf,
                alpha = alpha),
            fill = fill_blue) +
  scale_alpha_identity() +
  geom_line(data = so2wt_daily_complete,
  ...
  scale_colour_manual(values = c("Low-income"  = col_low,
                      ...
                      name = NULL) +
  geom_text(data = ann_pct_so2,
            aes(x = make_date(year, 7, 1),
                y = max(c(so2wt_daily_complete$low,
                          so2wt_daily_complete$high), na.rm = TRUE) * 1.04,
                label = sprintf("%.0f%%", pct_low_gt_high)),
            size = 4.2, fontface = "bold", colour = "grey20") +
  scale_x_date(breaks = make_date(unique(ann_pct_so2$year), 1, 1),
               date_labels = "%Y",
               expand = expansion(mult = c(0.01, 0.08))) +
  labs(
    title = "Daily population-weighted SO2 by income group (California)",
...
ggsave(
  filename = file.path(out_dir, "so2_income_wildfire_2000-2023.pdf"),
  plot     = p_so2_wf,
...
ggsave(
  filename = file.path(out_dir, "so2_income_wildfire_2000-2010.pdf"),
  plot     = p_so2_0010,
  width    = 16, height = 7, units = "in"
)

ggsave(
  filename = file.path(out_dir, "so2_income_wildfire_2011-2023.pdf"),
  plot     = p_so2_1123,
  width    = 12, height = 7, units = "in"
)
