so2 <- read.csv("so2_california_2000_2023.csv")
...
stopifnot(!anyNA(so2_wpop$pop))  # should pass now

so2wt_daily <- so2_wpop %>% 
  group_by(date, income_grp) %>% 
  summarise(
    so2 = weighted.mean(so2, w = pop, na.rm = TRUE) %>% 
      replace_na(NA_real_),       # turn NaN → NA
    .groups = "drop"
  ) %>% 
  pivot_wider(names_from = income_grp, values_from = so2) %>% 
  mutate(disparity = high - low)       # absolute gap; use ratio if preferred

so2wt_daily_complete <- so2wt_daily %>% 
