# 1. Difference series -------------------------------------------------
diff_daily_ozone <- ozonewt_daily_complete |>
  transmute(
    date,
    diff = low - high
  ) |>
...
# 2. Plot --------------------------------------------------------------
p_diff_simple_ozone <- ggplot(diff_daily_ozone, aes(date, diff)) +
  ...
  labs(
    title = "Population-weighted daily Ozone difference (low - high income)",
    x     = NULL,
    y     = "Delta Ozone (ppm)"
  ) +
