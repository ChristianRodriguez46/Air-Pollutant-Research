ozone <- read.csv("ozone_california_2000_2023.csv")
...
ozone_wpop <- ozone_strat %>% 
  mutate(year = lubridate::year(date)) %>% 
  left_join(pop, by = c("county", "year"))

stopifnot(!anyNA(ozone_wpop$pop))  # should pass now

ozonewt_daily <- ozone_wpop %>% 
  group_by(date, income_grp) %>% 
  summarise(
    ozone = weighted.mean(ozone, w = pop, na.rm = TRUE) %>% 
      replace_na(NA_real_),       # turn NaN → NA
    .groups = "drop"
  ) %>% 
  pivot_wider(names_from = income_grp, values_from = ozone) %>% 
  mutate(disparity = high - low)       # absolute gap; use ratio if preferred

ozonewt_daily_complete <- ozonewt_daily %>% 
