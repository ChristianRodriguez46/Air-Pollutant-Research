# 1. Difference series -------------------------------------------------
diff_daily_no2 <- no2wt_daily_complete |>
  transmute(
    date,
    diff = low - high
  ) |>
...
# 2. Plot --------------------------------------------------------------
p_diff_simple_no2 <- ggplot(diff_daily_no2, aes(date, diff)) +
  ...
  labs(
    title = "Population-weighted daily NO2 difference (low - high income)",
    x     = NULL,
    y     = "Delta NO2 (ppb)"
  ) +
